...
#shuffle gene expression (shuffle sample IDs, keep same distribution of exp per gene, 
#keeps exp correlation together, will separate from genotypes)
shuffle <- sample(rownames(exp.w.geno)) #shuffle rownames (IDs)
rownames(exp.w.geno) <- shuffle #assign shuffled IDs to exp.w.geno
...
write.table(output,file=tis %&% ".local-h2_gen-corr.SAMPLE-" %&% seednum %&% "." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
