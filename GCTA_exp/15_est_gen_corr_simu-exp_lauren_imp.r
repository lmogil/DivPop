...
#args <- c('AFA','CAU','1') #uncomment for testing
...
rna.dir <- out.dir %&% "MESA_simulated_exp/"
...
for(pop in c(pop1, pop2)){
  expfile <- rna.dir %&% pop1 %&% "-" %&% pop2 %&% "_MESA_Nk-20.local-h2_gen-corr_UMich_imp.2018-05-03_" %&% 
    pop %&% "-EXP-sim" %&% simnum %&% ".txt"
...
for(pop in c(pop1, pop2)){
  famfile <- gt.dir %&% "MESA_" %&% pop %&% "_w_expression.fam" ###samples with gt data###
  fam <- read.table(famfile)
  samplelist <- intersect(fam$V2,expsamplelist)
...
system("ls " %&% grm.dir %&% pop1 %&% "-" %&% pop2 %&% "-local*id > tmp." %&% pop1 %&% pop2 %&% simnum)
system("awk -F \"" %&% pop1 %&% "-" %&% pop2 %&% "-local-\" \'{print $2}\' < tmp." %&% pop1 %&% pop2 %&% simnum %&%
...
for(i in 1:length(locallist$gene)){
  ...
  write.table(geneexp, file="tmp." %&% pop1 %&% pop2 %&% simnum %&% ".pheno", col.names=F, quote=F) #output pheno for gcta
  
  ## Y ~ localGRM
  runLOC <- "/usr/local/bin/gcta64 --grm " %&% grm.dir %&% pop1 %&% "-" %&% pop2 %&% "-local-" %&% gene %&% 
    " --reml-bivar --pheno tmp." %&% pop1 %&% pop2 %&% simnum %&% ".pheno --out tmp." %&% pop1 %&% pop2 %&% simnum %&% " --thread-num 1"
  system(runLOC)
  if(file.exists("tmp." %&% pop1 %&% pop2 %&% simnum %&% ".hsq")==TRUE){
    hsq <- scan("tmp." %&% pop1 %&% pop2 %&% simnum %&% ".hsq","character")
    ...
    system("rm tmp." %&% pop1 %&% pop2 %&% simnum %&% ".hsq")
  }else{
...
write.table(output,file=out.dir %&% tis %&% ".local-h2_gen-corr_EXP-SIM-" %&% simnum %&% "_UMich_imp.2018-05-03.txt",quote=F,row.names=F,sep="\t")
