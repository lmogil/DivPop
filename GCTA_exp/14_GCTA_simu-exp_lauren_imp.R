...
#pop2 <- 'CAU'
#simnum <- 1
resfile <- out.dir %&% pop1 %&% "-" %&% pop2 %&% "_MESA_Nk-20.local-h2_gen-corr_UMich_imp.2018-05-03.txt"
...
fwrite(tpop1df, file=out.dir %&% "MESA_simulated_exp/" %&% pop1 %&% "-" %&% pop2 %&% 
         "_MESA_Nk-20.local-h2_gen-corr_UMich_imp.2018-05-03_" %&% pop1 %&% "-EXP-sim" %&% simnum %&% ".txt", sep="\t")

tpop2 <- t(pop2.mat)
tpop2df <- as.data.frame(tpop2) %>% rownames_to_column(var="PROBE_ID")
fwrite(tpop2df, file=out.dir %&% "MESA_simulated_exp/" %&% pop1 %&% "-" %&% pop2 %&% 
         "_MESA_Nk-20.local-h2_gen-corr_UMich_imp.2018-05-03_" %&% pop2 %&% "-EXP-sim" %&% simnum %&% ".txt", sep="\t")

fwrite(res, file=out.dir %&% pop1 %&% "-" %&% pop2 %&% "_MESA_Nk-20.local-h2_gen-corr_UMich_imp.2018-05-03_with_SNP_count.txt",na=NA,sep="\t")
