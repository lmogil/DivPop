...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
                 ...
                 seed=NA, cis_window=1e6, alpha=1, null_testing=FALSE) {
  ...
  # Prepare output data----
  model_summary_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output_alphas/' %&% prefix %&% '_chr' %&% chrom %&% '_model_summaries_' %&% peer %&% '_peer_' %&% covar %&% 'pcs_alpha1.txt'
  ...
  weights_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output_alphas/' %&% prefix %&% '_chr' %&% chrom %&% '_weights_' %&% peer %&% '_peer_' %&% covar %&% 'pcs_alpha1.txt'
  weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
  write(weights_col, file = weights_file, ncol = 6, sep = '\t')
  
  tiss_chr_summ_f <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output_alphas/' %&% prefix %&% '_chr' %&% chrom %&% '_tiss_chr_summary_' %&% peer %&% '_peer_' %&% covar %&% 'pcs_alpha1.txt'
  ...
  covariance_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output_alphas/' %&% prefix %&% '_chr' %&% chrom %&% '_covariances_' %&% peer %&% '_peer_' %&% covar %&% 'pcs_alpha1.txt'
  ...
  for (i in 1:n_genes) {
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      fit <- tryCatch(cv.glmnet(cis_gt, adj_expression, nfolds = n_folds, alpha = 1, type.measure='mse', foldid = cv_fold_ids, keep = TRUE),
