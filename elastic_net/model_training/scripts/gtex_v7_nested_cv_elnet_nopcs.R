...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file, chrom, prefix, maf=0.01, n_folds=10, n_train_test_folds=5,
 ...
 #covariates_df <- get_covariates(covariates_file, samples)
  ...
  # Prepare output data----
  model_summary_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output/' %&% prefix %&% '_chr' %&% chrom %&% '_model_summaries_nopc.txt'
  ...
  weights_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output/' %&% prefix %&% '_chr' %&% chrom %&% '_weights_nopc.txt'
  weights_col <- c('gene_id', 'rsid', 'varID', 'ref', 'alt', 'beta')
  write(weights_col, file = weights_file, ncol = 6, sep = '\t')
  
  tiss_chr_summ_f <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output/' %&% prefix %&% '_chr' %&% chrom %&% '_tiss_chr_summary_nopc.txt'
  ...
  covariance_file <- '/home/lauren/files_for_revisions_plosgen/en_v7/new_output/' %&% prefix %&% '_chr' %&% chrom %&% '_covariances_nopc.txt'
  ...
  for (i in 1:n_genes) {
    ...
    if (ncol(cis_gt) >= 2) {
      ...
      adj_expression <- scale(expression_vec, center = TRUE, scale = TRUE)
      if (null_testing)
