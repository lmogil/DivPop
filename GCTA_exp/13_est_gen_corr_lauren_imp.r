...
rna.dir <- pre %&% "/lauren/files_for_revisions_plosgen/expresion/old_exp/"
...
for(pop in c(pop1, pop2)){
  expfile <- rna.dir %&% "meqtl_sorted_" %&% pop %&% "_MESA_Epi_GEX_data_sidno_Nk-20.txt"
...
gencodefile <- pre %&% '/lauren/mesa_expression_files_ens/MESA_GEX_ilmn_gencode.v18.annotation.txt'
...
write.table(output,file=tis %&% ".local-h2_gen-corr_UMich_imp." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
