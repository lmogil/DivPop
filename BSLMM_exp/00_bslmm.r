...
working100K <- out.dir %&% "working_" %&% tis %&% "_exp_BSLMM-s100K_iterations_chr" %&% chrom %&% "_" %&% date %&% ".txt"
...
for(i in 1:length(explist)){
  ...
  if(dim(cisgenos)[1] > 0){
    ...
    write.table(annotfile, file=out.dir %&% "tmp2.annot." %&% pop %&% chrom , quote=F, row.names=F, col.names=F, sep=",")
    ...
    runBSLMM <- "/usr/local/bin/gemma -g " %&% out.dir %&% "tmp2.geno." %&% pop %&% chrom %&%  " -p " %&% out.dir %&% "tmp2.pheno." %&% 
      pop %&% chrom %&% " -a " %&% out.dir %&% "tmp2.annot." %&% pop %&% chrom %&% " -bslmm 1 -seed 12345 -s 100000 -o tmp2." %&% pop %&% chrom  
    system(runBSLMM)

    hyp <- read.table(out.dir %&% "output/tmp2." %&% pop %&% chrom %&% ".hyp.txt",header=T)
...
write.table(resultsarray,file=out.dir %&% tis %&% "_exp_BSLMM-s100K_iterations_chr" %&% chrom %&% "_" %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
