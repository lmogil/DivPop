...
for(pop in c(pop1, pop2)){
  expfile <- rna.dir %&% pop1 %&% "-" %&% pop2 %&% "_MESA_Nk-20.local-h2_gen-corr.2017-07-21_" %&% 
...
for(i in 1:length(locallist$gene)){
  ...
  runLOC <- "/usr/local/bin/gcta64 --grm " %&% grm.dir %&% pop1 %&% "-" %&% pop2 %&% "-local-" %&% gene %&% 
    " --reml-bivar --pheno tmp." %&% pop1 %&% pop2 %&% simnum %&% ".pheno --out tmp." %&% pop1 %&% pop2 %&% simnum %&% " --thread-num 10"
  system(runLOC)
...
write.table(output,file=tis %&% ".local-h2_gen-corr_EXP-SIM-" %&% simnum %&% "." %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
